import React from 'react'
import icon2 from '../assets/images/png/MS/Isolation_Mode.png'
import icon1 from '../assets/images/png/MS/icon1.png'

const Blogs = ({src ,date, comments, title, description}) => {
  return (
    <>
        <div className='border border-[#00000026] font-noto'>
            <div className='relative'>
                <img  src={src} alt="img" className=''/>
                <p className='bg-[#4848484D] p-3 text-white backdrop-blur-md absolute bottom-0 right-0'>
                    {date}</p>
            </div>

            <div className='px-6 py-8'>
            <div className='flex items-center gap-2 mb-3'>
                <img src={icon1}/>
                <p>{comments}</p>
            </div>
            <div className='flex flex-col gap-3' >
                <h5 className='text-lg'>{title}</h5>
                <p className='text-md'>{description}.</p>
                <div className='flex items-center gap-4'>
                    <p className='text-[#D1B271]'>Read More</p>
                    <img src={icon2} alt=''/>  
                </div>
            </div>
            </div>


        </div>
    </>
  )
}

export default Blogs